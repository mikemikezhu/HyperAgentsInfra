#!/bin/bash
#SBATCH --job-name=ha-pr-gvf-l10-s0-long-q38
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
if [[ -n "$stop_token_budget" ]]; then
    ...
    job_id="$(
        sbatch \
            --parsable \
            --job-name="ha-pr-a3-l10-s0-g0-$first_generation_target-q38" \
            "$script_path" \
            long-run-start \
...
if [[ "$#" -eq 0 ]]; then
    ...
    printf 'A3 lambda=10 sibling=0: %s\n' "$job_chain"
    exit 0
fi
...
exec "$launcher_path" gvf_lambda10_sibling0 "$@"
