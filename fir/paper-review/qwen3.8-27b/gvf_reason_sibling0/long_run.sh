#!/bin/bash
#SBATCH --job-name=ha-pr-gvf-reason-s0-long-q38
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
if [[ -n "$stop_token_budget" ]]; then
    ...
    job_id="$(
        sbatch \
            --parsable \
            --job-name="ha-pr-a4-s0-g0-$first_generation_target-q38" \
            "$script_path" \
            long-run-start \
    ...
    printf 'A4 sibling=0 token budget %s: %s\n' "$stop_token_budget" "${job_id%%;*}"
    exit 0
fi
    ...
    do
        ...
        job_id="$(
            sbatch \
                --parsable \
                "${dependency[@]}" \
                --job-name="ha-pr-a4-s0-g${generation_range}-q38" \
                "$script_path" \
                "$phase" \
                "$max_generation"
        )"
...
exec "$launcher_path" gvf_reason_sibling0 "$@"
