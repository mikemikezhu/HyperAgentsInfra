#!/bin/bash
#SBATCH --job-name=ha-pr-gvf-l1-s1-long-q38
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
if [[ -n "$stop_token_budget" ]]; then
    ...
    job_id="$(
        sbatch \
            --parsable \
            --job-name="ha-pr-a3-l1-s1-g0-$first_generation_target-q38" \
            "$script_path" \
            long-run-start \
    ...
    printf 'A3 lambda=1 sibling=1 token budget %s: %s\n' "$stop_token_budget" "${job_id%%;*}"
    exit 0
fi
...
if [[ "$#" -eq 0 ]]; then
    ...
    do
        ...
        job_id="$(
            sbatch \
                --parsable \
                "${dependency[@]}" \
                --job-name="ha-pr-a3-l1-s1-g${generation_range}-q38" \
                "$script_path" \
                "$phase" \
                "$max_generation"
        )"
    ...
    printf 'A3 lambda=1 sibling=1: %s\n' "$job_chain"
    exit 0
fi
...
exec "$launcher_path" gvf_lambda1_sibling1 "$@"
