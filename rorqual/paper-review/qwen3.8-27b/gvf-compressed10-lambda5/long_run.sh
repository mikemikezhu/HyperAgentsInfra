#!/bin/bash
#SBATCH --job-name=ha-pr-gvf-c10-l5-long-q38
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
if [[ -n "$stop_token_budget" ]]; then
    ...
    job_id="$(
        sbatch \
            --parsable \
            --job-name="ha-pr-a3-l5-g0-$first_generation_target-q38" \
            "$script_path" \
            long-run-start \
    ...
    printf 'A3 lambda=5 token budget %s: %s\n' "$stop_token_budget" "${job_id%%;*}"
    exit 0
fi
    ...
    do
        ...
        job_id="$(
            sbatch \
                --parsable \
                "${dependency[@]}" \
                --job-name="ha-pr-a3-l5-g${generation_range}-q38" \
                "$script_path" \
                "$phase" \
                "$max_generation"
        )"
...
exec "$launcher_path" gvf-compressed10-lambda5 "$@"
